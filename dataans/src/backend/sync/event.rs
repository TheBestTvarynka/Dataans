...
// https://github.com/JonasKruckenberg/tauri-sys/blob/e953a9c912212d43fa42add71bc84c40c30140e0/src/event.rs
    ...
    T: DeserializeOwned + 'static,
{
    ...
    let unlisten = inner::listen(event, &closure)
        .await
        .map_err(|err| {
            error!("{:?}", err);
            CommandError::TauriEvent(format!("{:?}", err))
    ...
    T: DeserializeOwned + 'static,
{
    ...
    let unlisten = inner::once(event, &closure)
        .await
        .map_err(|err| {
            error!("{:?}", err);
            CommandError::TauriEvent(format!("{:?}", err))
...
mod inner {
    ...
    extern "C" {
        ...
        #[wasm_bindgen(catch)]
        pub async fn listen(event: &str, handler: &Closure<dyn FnMut(JsValue)>) -> Result<JsValue, JsValue>;
        #[wasm_bindgen(catch)]
        pub async fn once(event: &str, handler: &Closure<dyn FnMut(JsValue)>) -> Result<JsValue, JsValue>;
    }
}
